sap.ui.define([
	...
	return BaseController.extend("defence.finance.finux.gr.controller.GR", {
		...
		onInit: function () {
			...
			// Matched GR edit selection route
		...
		onSubmit: function () {
			...
			oData.CancelReason = oViewModel.getProperty("/cancelReason");
						...
						var dialog = new sap.m.Dialog({
							...
							beginButton: new sap.m.Button({
								...
								press: function (oEvent1) {
									...
									// If rating or comments provided, save asynchronously (fire-and-forget)
									...
									// Close dialog and navigate immediately - do not wait for rating save
									dialog.close();
		...
		onMorePress: function (oEvent) {
			if (!this._actionSheet) {
				this._actionSheet = sap.ui.xmlfragment(this.getView().getId(), "defence.finance.finux.gr.view.GRMoreActionSheet", this);
		...
		getGRSelectDialog: function () {
			if (!this.oGRSelectDialog) {
				this.oGRSelectDialog = sap.ui.xmlfragment("defence.finance.finux.gr.view.GRSelectDialog", this);
				this.getView().addDependent(this.oGRSelectDialog);
				syncStyleClass(this.getOwnerComponent().getContentDensityClass(), this.getView(), this.oGRSelectDialog);
			}
		...
		onNavHome: function() {				
			...
			if (bFLP === true) {
		        ...
		        // Try to navigate back, fallback to home if no history
		...
		onGRValueHelpRequested: function(oEvent) {
			...
			if (! this.oValueHelpDialog ) {
				this.oValueHelpDialog = sap.ui.xmlfragment("defence.finance.finux.gr.view.GRValueHelp", this);
		...
		onItemSelectionChanged: function (oEvent) {
			// Get the table from the event source
